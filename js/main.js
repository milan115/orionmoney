...
function initPage () {
  ...
  function initSelect2 (data) {
    select.select2({
      data,
      templateResult (data) {
        return $('<span>' + data.desc + '</span><span>' + data.text + '<span>')
      }
    }).change(function () {
      formCounter()
    })
  ...
  function formCounter () {
    ...
    $('#interest_crypto .amount').text(Number(deposit_interest).toLocaleString())
    ...
    $('#total .amount').text(Number(deposit_amount + deposit_interest).toLocaleString())
  }
...
//tooltip
  ...
  //scrollTo
  $(document).on('click', '.nav a[href^="#"], a.scroll[href^="#"]', function (event) {
